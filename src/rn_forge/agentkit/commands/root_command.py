"""Implement `agentkit diff/doctor/version`.

These combine adapter selection with core resolution, diagnostics, and structural diff
helpers without mutating managed or native files.
"""
...
class RootCommand(BaseCommand):
    """Implement the root-level ``diff``, ``doctor``, and ``version`` commands."""
    ...
    def diff(
        ...
        if self.json_output:
            ...
            for item in records:
                self._render_diff(item, scope=scope, show_all=show_all)
        if check and drift:
            raise typer.Exit(2)

    def _render_diff(self, item: dict[str, Any], *, scope: str, show_all: bool) -> None:
        """Print one agent's capture summary, overriding keys, and per-file drift."""
        console.print(f"[bold]{item['agent']}[/bold] ({scope})")
        self._print_capture_summary(item, scope=scope)
        self._print_layer_changes(item, show_all=show_all)
        self._print_artifact_drift(item)

    def _print_capture_summary(self, item: dict[str, Any], *, scope: str) -> None:
        if item["capture"] is not None and item["capture"]["changed"]:
            ...
            console.print(
                f"[dim]Nothing to capture: {item['capture']['message']}.[/dim]"
            )
        ...
        if item["capture"] is not None and (
            ...
            or (
                item["capture_defaults"] is not None
                and item["capture_defaults"]["changed"]
            )
        ):
    ...
    def _print_layer_changes(self, item: dict[str, Any], *, show_all: bool) -> None:
        changes = item["layers"]
        shown = (
            changes if show_all else [c for c in changes if c["layer"] != "defaults"]
        )
        if shown:
            table = Table(
                "Key", "Layer", "Before", "After", title="Configuration layers"
            )
    ...
    def _print_artifact_drift(self, item: dict[str, Any]) -> None:
    ...
    def doctor(
        ...
        if self.json_output:
            ...
            artifact_rows = self._render_doctor(
    ...
    def _render_doctor(
        ...
        *,
        ...
        """
        self._print_agent_summary(results)
        ...
        self._print_artifact_rows(artifact_rows, roots, used)
        self._print_diagnostic_rows(diagnostic_rows)
        self._print_doctor_footer(results, shown, show_all)

        return artifact_rows

    def _print_agent_summary(self, results: list[CheckResult]) -> None:
        for agent in sorted({item.agent for item in results if item.agent}):
            owned = [item for item in results if item.agent == agent]
            counts = Counter(
                item.severity for item in owned if item.status not in HEALTHY
            )
            parts = [
    ...
    def _print_diagnostic_rows(self, diagnostic_rows: list[CheckResult]) -> None:
        diagnostics = Table(
    ...
    def _print_doctor_footer(
        self, results: list[CheckResult], shown: list[CheckResult], show_all: bool
    ) -> None:
    ...
    def version(self) -> None:
        ...
        adapter_versions: dict[str, str] = {
            adapter.name: adapter.version for adapter in self.selected(None)
        }
        data: dict[str, Any] = {
