...
from .base import BaseCommand, console, fail
...
class SelfCommand(BaseCommand):
    ...
    def cleanup(self, yes: bool, quiet: bool) -> None:
        """Remove every installed agentkit version except the one `current` points
        to."""
        product_home = _product_home()
        current_name = _current_version_dir(product_home)
        if current_name is None:
            self.fail(f"no current install found at {product_home / 'current'}")
    ...
    def uninstall(self, yes: bool, dry_run: bool) -> None:
        """Undo the global install: strip hook wiring, remove owned files, then
        agentkit.

        Removes agentkit's hook registrations from each adapter's native global config
        (Claude's `~/.claude/settings.json`, Codex's `~/.codex/hooks.json`) — leaving
        the rest, like permission grants and output style, untouched — deletes the skill
        files agentkit wrote under `~/.claude` and `~/.codex`, then removes the
        installed versions under `$RNF_HOME/agentkit/` and the `agentkit` command
        itself.
        ...
        step here nothing can undo. Repository-local installs from `agentkit project
        init` are untouched — run `agentkit project remove` in each one.
        """
        adapters = self.selected(None)
        ...
        with self.boundary():
            for adapter in adapters:
                ...
                results.extend(
                    remove_owned_artifacts(
                        adapter, "global", repo_root, dry_run=dry_run
                    )
                )
